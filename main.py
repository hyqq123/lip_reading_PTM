...
def load_args(default_config=None):
    ...
    # -- directory
    parser.add_argument('--data-dir', default=r'D:\hy\LRW_npz\visual_data_Table1', help='Loaded data directory')
    parser.add_argument('--label-path', type=str, default='./labels/500WordsSortedList.txt',
                        help='Path to txt file with labels')
    parser.add_argument('--annonation-direc', default=r'D:\hy\LRW\lipread_mp4', help='Loaded data directory')
    # -- model config
    ...
    # -- DenseTCN config
    parser.add_argument('--densetcn-block-config', type=int, nargs="+",
                        help='number of denselayer for each denseTCN block')
    parser.add_argument('--densetcn-kernel-size-set', type=int, nargs="+",
    ...
    parser.add_argument('--init-epoch', default=65, type=int, help='epoch to start at')
    ...
    # -- test
    parser.add_argument('--model-path', type=str, default='./models/ckpt.best.pth', help='Pretrained model pathname')
    ...
    # -- json pathname
    parser.add_argument('--config-path', type=str, default='./configs/lrw_resnet18_dctcn_boundary.json',
...
def main():
    ...
    if args.model_path:
        ...
        if args.init_epoch > 0:
            ...
            # model, optimizer,  ckpt_dict = load_model(args.model_path, model, optimizer)
            model, optimizer, epoch_idx, ckpt_dict = load_model(args.model_path, model, optimizer)
            # 继续上次训练，当使用训练好的模型训练时注释这段代码
    ...
    while epoch < args.epochs:
        ...
        acc_avg_val, loss_avg_val = evaluate(model, dset_loaders['test'], criterion)
        logger.info(
