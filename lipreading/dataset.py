...
class MyDataset(object):
    def __init__(self, modality, data_partition, data_dir, label_fp, annonation_direc=None,
        ...
        self._annonation_direc = r'D:\hy\LRW\lipread_mp4'
    ...
    def _get_files_for_partition(self):
        ...
        self._data_files = [ f for f in self._data_files if f.replace('\\', '/').split('/')[self.label_idx] in self._labels ]
    ...
    def _apply_variable_length_aug(self, filename, raw_data):
        ...
        # print(filename)
        # info_txt = r'G:/Date/lrw-v1/lipread_mp4/' + '/'.join(filename.replace('\\', '/').split('/')[self.label_idx:]) # swap base folder
        info_txt = os.path.join(self._annonation_direc, *filename.replace('\\', '/').split('/')[self.label_idx:] )
